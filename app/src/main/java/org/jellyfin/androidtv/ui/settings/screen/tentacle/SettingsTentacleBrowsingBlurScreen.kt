...
@Composable
fun SettingsTentacleBrowsingBlurScreen() {
	val router = LocalRouter.current
	val userSettingPreferences = koinInject<UserSettingPreferences>()
	var browsingBackgroundBlurAmount by rememberPreference(userSettingPreferences, UserSettingPreferences.browsingBackgroundBlurAmount)
	...
	SettingsColumn {
		item {
			ListSection(
				...
				captionContent = { Text(stringResource(R.string.pref_browsing_background_blur_amount_description)) },
			)
		}
		...
		items(options) { (value, label) ->
			ListButton(
				...
				trailingContent = { RadioButton(checked = browsingBackgroundBlurAmount == value) },
				onClick = {
					browsingBackgroundBlurAmount = value
					router.back()
				}
			)
		}
