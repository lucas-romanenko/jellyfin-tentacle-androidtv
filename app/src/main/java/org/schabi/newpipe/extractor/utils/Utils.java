...
public final class Utils {
    ...
    public static long mixedNumberWordToLong(final String numberWord)
        ...
        String multiplier = "";
        try {
            multiplier = Parser.matchGroup("[\\d]+([\\.,][\\d]+)?([KMBkmb])+",
                    numberWord, 2);
        } catch (final ParsingException ignored) {
        }
        final double count = Double.parseDouble(
                Parser.matchGroup1("([\\d]+([\\.,][\\d]+)?)", numberWord)
                        .replace(",", "."));
     ...
     */
    public static void checkUrl(final String pattern, final String url)
     ...
     * @param url     the url to be tested
     */
    public static void checkUrl(final Pattern pattern, final String url)
            throws ParsingException {
     ...
     *
     * <p>
     * If an url-query is give multiple times, only the value of the first
     * query is returned.
     * </p>
     *
     * @param url           the url to be used
     * @param parameterName the pattern that will be used to check the url
     * @return a string that contains the value of the query parameter or
    ...
    /**
     * If the provided url is a Google search redirect, then the actual url is
     * extracted from the {@code url=} query value and returned, otherwise the
    ...
    public static String followGoogleRedirectIfNeeded(final String url) {
        try {
            ...
            if (decoded.getHost().contains("google")
                    && decoded.getPath().equals("/url")) {
                return decodeUrlUtf8(
                        Parser.matchGroup1("&url=([^&]+)(?:&|$)", url));
            }
     ...
     *
     * <p>
     * This method can be also used for
     * {@link com.grack.nanojson.JsonArray JsonArray}s.
     * </p>
     *
     ...
     * <p>
     * This method can be also used for
     * {@link com.grack.nanojson.JsonObject JsonObject}s.
     * </p>
     *
    ...
    /**
     * Find the result of an array of string regular expressions inside an
     * input on the first group ({@code 0}).
     *
    ...
    /**
     * Find the result of an array of {@link Pattern}s inside an input on the
     * first group ({@code 0}).
     *
     * @param input   the input on which using the regular expressions
     ...
     * @return the result
     * @throws Parser.RegexException if none of the patterns match the input,
     *         or at least in the specified group
     */
    @Nonnull
    public static String getStringResultFromRegexArray(@Nonnull final String input,
                                                       @Nonnull final String[] regexes,
    ...
    /**
     * Find the result of an array of {@link Pattern}s inside an input on a
     * specific group.
     *
     * @param input   the input on which using the regular expressions
     * @param regexes the {@link Pattern} array
