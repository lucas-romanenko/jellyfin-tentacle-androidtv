...
class HomeRowsFragment : RowsSupportFragment(), AudioEventListener, View.OnKeyListener {
				...
				for (section in allSections) {
					...
					when (section.type) {
						...
						"builtin" -> {
							...
							addBuiltInSection(rows, sectionId, includeLiveTvRows)
						}
					}
				}
			} else {
	...
	private suspend fun addBuiltInSection(
		...
		// Accepts both Android TV names (e.g. "resume") and Jellyfin web names (e.g. "resumevideo")
		when (sectionId) {
			"latestmedia" -> rows.add(helper.loadRecentlyAdded(userViewsRepository.views.first()))
